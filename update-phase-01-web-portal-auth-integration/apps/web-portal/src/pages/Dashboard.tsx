...
import { aiPoliciesApi, type EffectivePolicy } from '../lib/api/aiPolicies';
...
export function Dashboard() {
  ...
  const [effectivePolicies, setEffectivePolicies] = useState<EffectivePolicy[]>([]);
  ...
  const fetchEffectivePolicies = async () => {
    setPolicyError('');
    setPolicyLoading(true);
    try {
      const response = await aiPoliciesApi.getEffective();
      const responseData = (response as { data?: EffectivePolicy[] | EffectivePolicy }).data;
      if (Array.isArray(responseData)) {
        setEffectivePolicies(responseData);
      ...
      setEffectivePolicies([]);
      setPolicyError(error instanceof Error ? error.message : 'Failed to load AI policies');
    } finally {
    ...
    <div className="space-y-5">
      ...
      <div className="card p-4">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-base font-semibold">سياسات الذكاء الاصطناعي الفعالة</h2>
            <p className="text-sm text-white/60">يتم جلب البيانات مباشرة من /api/v1/ai-policies/effective بعد تسجيل الدخول</p>
          </div>
          <button
            onClick={fetchEffectivePolicies}
            disabled={policyLoading}
            className="btn-secondary text-sm px-4 py-2"
          >
            {policyLoading ? '...جاري التحديث' : 'تحديث' }
          </button>
        </div>

        {policyError && (
          <div className="p-3 rounded-lg border border-red-500/40 bg-red-500/10 text-sm text-red-300 mb-3">
            {policyError}
          </div>
        )}

        {!policyLoading && effectivePolicies.length === 0 && !policyError && (
          <p className="text-white/60 text-sm">لا توجد سياسات فعالة للعرض.</p>
        )}

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {effectivePolicies.map((policy, idx) => (
            <div key={policy.id || idx} className="p-3 rounded-lg bg-white/5 border border-white/10">
              <div className="flex items-center justify-between gap-2 mb-2">
                <p className="font-semibold truncate">{policy.name || 'سياسة بدون اسم'}</p>
                {policy.status && (
                  <span className="badge px-2 py-0.5 text-[11px] bg-stc-gold/20 text-stc-gold border border-stc-gold/40">
                    {String(policy.status)}
                  </span>
                )}
              </div>
              <p className="text-xs text-white/60 mb-2">
                {policy.scope ? `نطاق التطبيق: ${policy.scope}` : 'النطاق غير محدد'}
              </p>
              <div className="bg-black/30 rounded-lg p-3 text-xs font-mono overflow-x-auto">
                <pre className="whitespace-pre-wrap break-words text-white/80">{JSON.stringify(policy, null, 2)}</pre>
              </div>
            </div>
          ))}
        </div>
